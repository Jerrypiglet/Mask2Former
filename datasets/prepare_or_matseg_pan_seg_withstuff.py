...
im_matseg_pan_seg_rgb_withstuff_root = Path('/data/OpenRooms_public/im_matseg_pan_seg_rgb_withstuff')
frame_list_root = project_root_path / 'data/openrooms/public'

OR_MATSEG_2_CATEGORIES = [
    {
...
if __name__ == "__main__":
    ...
    for split in ['train']:
        ...
        out_folder = os.path.join(dataset_dir, f"OpenRooms_public/im_matseg_pan_seg_rgb_withstuff/")
        # json with segmentations information
        out_file = os.path.join(dataset_dir, f"OpenRooms_public/im_matseg_pan_seg_rgb_withstuff_{split}.json")
        ...
        categories_dict = {cat["id"]: cat for cat in OR_MATSEG_2_CATEGORIES}

        panoptic_json_categories = OR_MATSEG_2_CATEGORIES[:]
        ...
        for image_id, im_info_dict in tqdm(enumerate(im_info_list)):
            ...
            im_matseg_pan_seg_rgb_withstuff_folder = im_matseg_pan_seg_rgb_withstuff_root / (im_info_dict['meta_split'].replace('DiffMat', '').replace('DiffLight', '')) / im_info_dict['scene_name']
            im_matseg_pan_seg_rgb_withstuff_folder.mkdir(parents=True, exist_ok=True)
            im_matseg_pan_seg_rgb_withstuff_path = im_matseg_pan_seg_rgb_withstuff_folder / ('im_matseg_pan_seg_rgb_withstuff_%s.png'%im_info_dict['frame_id'])
            
            # if im_matseg_pan_seg_rgb_withstuff_path.exists():
            #     continue
            
            print(im_matseg_pan_seg_rgb_withstuff_path)
            ...
            panoptic_json_annotation = {
                "image_id": image_id,
                "file_name": str(im_matseg_pan_seg_rgb_withstuff_path.relative_to(or_root/'im_matseg_pan_seg_rgb_withstuff')), 
                "image_file_name": str(im_info_dict['im_sdr_path'].relative_to(or_root)), 
                # "file_name": str(im_matseg_pan_seg_rgb_withstuff_path.relative_to(or_root/'im_matseg_pan_seg_rgb_withstuff')), 
                "pan_seg_file_name": str(im_matseg_pan_seg_rgb_withstuff_path.relative_to(or_root/'im_matseg_pan_seg_rgb_withstuff')), 
            ...
            # if not im_matseg_pan_seg_rgb_withstuff_path.exists():
            # print('WRITTT ->', str(im_matseg_pan_seg_rgb_withstuff_path))
            im_matseg_pan_seg_rgb_withstuff = panopticapi_utils.id2rgb(im_matseg_pan_seg_ids)
            cv2.imwrite(str(im_matseg_pan_seg_rgb_withstuff_path), im_matseg_pan_seg_rgb_withstuff[:, :, ::-1])
            # else:
            #     print('READDDD: ', str(im_matseg_pan_seg_rgb_withstuff_path))
            #     im_matseg_pan_seg_rgb_withstuff = cv2.imread(str(im_matseg_pan_seg_rgb_withstuff_path))[::-1]
            #     pass
